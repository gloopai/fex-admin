...
不做 OKX Smart Portfolio 那类真实比例调仓机器人，也不做 Binance Auto-Invest 那类定投计划。原因是参考图的核心信息是投资区间、运行天数、收益区间和认购流程，更接近固定期限理财产品，与当前项目已有 AI 量化模块的业务和视觉结构更一致。
...
- 固定期限收益产品：用户投入单一计价币种，平台展示组合币种、周期、收益区间、费用和赎回规则，运营后台控制产品参数。
...
- 列表页提供“组合市场 / 我的组合”切换。
- 卡片内展示组合币种、周期、日收益区间、金额区间、状态和 CTA。
...
- 产品管理为筛选器 + 表格 + 编辑弹窗。
- 编辑弹窗采用左侧配置、右侧实时预览。
- 配置分 Tab：基础信息、组合币种、收益与费用、限额与赎回、展示文案。
