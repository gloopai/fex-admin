      ...
      <div class="space-y-5 px-6 py-5">
        ...
        <div v-show="activeTemplateTab === 'contract'" class="space-y-4">
          ...
          <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            ...
            <p>- 按数量(张数=币价)下单：用户输入的是张数，张数按币价口径填写，系统根据合约面值和杠杆计算需要占用的保证金。</p>
            <p class="mt-3 font-medium text-slate-800">保证金计算规则</p>
            <p class="mt-2">- 按成本模式：保证金 = 用户输入成本。</p>
            <p>- 按数量模式：保证金 = 张数(币价口径) × 合约面值 ÷ 杠杆倍数。</p>
            <p class="mt-2 text-amber-700">合约面值用于张数(币价口径)与 USDT 名义价值之间的换算，例如合约面值 1000 USDT 时，1 张代表 1000 USDT 名义价值。</p>
          </div>
        </div>
      </div>
