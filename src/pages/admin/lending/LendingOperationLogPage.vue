  ...
  <section class="space-y-4">
    <header>
      ...
      <p class="mt-1 text-sm text-slate-500">
        记录授信额度/评分卡、借贷产品与订单审核等关键操作，便于审计与追溯（演示数据可随页面操作追加）。
      </p>
    </header>
    ...
    <article class="rounded-xl border border-slate-200 bg-white">
      <div class="flex flex-wrap items-center gap-3 border-b border-slate-200 p-4">
        ...
        >
          ...
          <option v-for="(label, key) in LENDING_OP_MODULE_LABEL" :key="key" :value="key">{{ label }}</option>
        </select>
        <input
          v-model="search"
          type="search"
            ...
            <tr v-for="row in pagedRows" :key="row.id" class="border-t border-slate-100">
              ...
              <td class="px-4 py-3">
                <span class="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">
                  {{ LENDING_OP_MODULE_LABEL[row.module] || row.module }}
                </span>
              </td>
              <td class="px-4 py-3 text-slate-800">{{ LENDING_OP_ACTION_LABEL[row.action] || row.action }}</td>
...
import {
  LENDING_OP_MODULE_LABEL,
  LENDING_OP_ACTION_LABEL
} from '../../../admin/constants/lendingOperationLog'
import { lendingOperationLogs } from '../../../admin/state/lendingOperationLogs'
...
const filteredRows = computed(() => {
  ...
  return lendingOperationLogs.value.filter((row) => {
