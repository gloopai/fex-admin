...
import { COMMON_FILTER_ALL, DELIVERY_CONTRACT_TAB, DELIVERY_STATUS, DELIVERY_VIEW_TAB } from '../../constants/delivery'
import { createDeliveryProductsMock, createDeliveryTemplatesMock } from '../../mock/delivery'

const viewTab = ref(DELIVERY_VIEW_TAB.PRODUCTS)
...
const saveTemplate = () => {
  ...
  if (editingTemplateId.value) {
    ...
    products.value = products.value.map((p) =>
      p.templateId === editingTemplateId.value ? { ...p, templateName: payload.name } : p
    )
  } else {
  ...
  <section class="space-y-4">
    <header class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">管理交割合约产品和周期模板配置</p>
      </div>
      <button
        type="button"
        class="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"
        @click="viewTab === DELIVERY_VIEW_TAB.PRODUCTS ? openCreateContract() : openCreateTemplate()"
      >
        <span class="text-base">+</span>
        <span>{{ viewTab === DELIVERY_VIEW_TAB.PRODUCTS ? '新增合约' : '新增模板' }}</span>
      </button>
    </header>
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 p-4">
        <div class="flex items-center gap-4">
          <div class="inline-flex rounded-lg border border-slate-200 bg-slate-50 p-1">
            <button type="button" class="rounded-md px-4 py-1.5 text-sm" :class="viewTab === DELIVERY_VIEW_TAB.PRODUCTS ? 'bg-white font-medium text-blue-600 shadow-sm' : 'text-slate-600'" @click="viewTab = DELIVERY_VIEW_TAB.PRODUCTS">合约产品</button>
            <button type="button" class="rounded-md px-4 py-1.5 text-sm" :class="viewTab === DELIVERY_VIEW_TAB.TEMPLATES ? 'bg-white font-medium text-blue-600 shadow-sm' : 'text-slate-600'" @click="viewTab = DELIVERY_VIEW_TAB.TEMPLATES">周期模板</button>
          </div>
        ...
        <input v-model="search" type="text" :placeholder="viewTab === DELIVERY_VIEW_TAB.PRODUCTS ? '搜索产品名称或代码...' : '搜索模板名称...'
        " class="w-full max-w-sm rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-blue-500" />
      </div>

      <div v-if="viewTab === DELIVERY_VIEW_TAB.PRODUCTS" class="space-y-3 p-4">
      ...
      <div v-else class="space-y-4 p-4">
        <article v-for="tpl in filteredTemplates" :key="tpl.id" class="rounded-xl border border-slate-200 bg-white">
          ...
          <div class="grid gap-3 p-4 md:grid-cols-3">
            <article v-for="(cycle, i) in tpl.cycles" :key="cycle.id" class="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <p class="text-sm text-slate-500">周期 {{ i + 1 }}</p>
              <p class="mt-2 flex items-center justify-between"><span class="text-slate-600">时长</span><span class="text-lg font-semibold text-slate-900">{{ durationLabel(cycle.durationSec) }}</span></p>
              <p class="mt-2 flex items-center justify-between"><span class="text-slate-600">收益率</span><span class="text-lg font-semibold text-emerald-600">{{ cycle.payoutPct.toFixed(2) }}%</span></p>
            </article>
          </div>
        </article>
      </div>
