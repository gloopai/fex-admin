          ...
          <div class="space-y-3">
            <div v-for="docType in singleImageDocKeys" :key="`single-${docType}`" class="rounded-2xl border border-white/10 bg-black/20 p-3">
              <div class="flex items-start justify-between gap-3">
                ...
                <span class="inline-flex items-center rounded-md border border-lime-300/40 bg-lime-300/10 px-2 py-0.5 text-[11px] leading-none text-lime-200">必传</span>
              </div>

              <label
                v-if="!form.files[docType]"
                class="mt-3 block cursor-pointer rounded-2xl border border-dashed border-white/25 bg-white/[0.04] p-5"
              >
              ...
              <div v-else class="mt-3 rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-3">
                ...
                <div class="mt-3 overflow-hidden rounded-xl border border-white/10 bg-black/30">
        ...
        <div class="rounded-2xl border border-white/10 bg-black/20 p-3">
          ...
          <label class="mt-3 block cursor-pointer rounded-2xl border border-dashed border-white/25 bg-white/[0.04] p-5">
            ...
            <input type="file" multiple accept="image/*,application/pdf" class="hidden" @change="onPickFile(docType, $event)" />
              ...
              <div v-if="(form.files[docType] || []).length" class="mt-3 space-y-2">
                <div v-for="(f, index) in form.files[docType]" :key="`${docType}-${index}-${f.name}`" class="rounded-xl border border-white/10 bg-black/30 p-3">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <div class="truncate text-sm">{{ f.name }}</div>
                      <div class="mt-1 text-xs text-white/55">{{ formatBytes(f.size) }}</div>
                    </div>
                    <button type="button" class="text-xs text-white/60" @click="clearFile(docType, index)">移除</button>
                  </div>

                  <div v-if="isImageFile(f)" class="mt-3 overflow-hidden rounded-xl border border-white/10">
                    <img :src="previewUrl(f)" alt="preview" class="h-44 w-full object-cover" />
                  </div>
