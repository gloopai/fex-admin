              ...
              <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div v-for="docType in singleImageDocKeys" :key="`single-${docType}`" class="rounded-2xl border border-white/10 bg-black/20 p-4">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="inline-flex items-center rounded-md border border-lime-300/40 bg-lime-300/10 px-2 py-0.5 text-xs leading-none text-lime-200">必传</span>
                  </div>

                  <label v-if="!advancedForm.singleFiles[docType]" class="mt-4 block cursor-pointer rounded-2xl border border-dashed border-white/25 bg-black/20 p-6 hover:bg-black/30">
                  ...
                  <div v-else class="mt-3 rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-3">
                    <div class="flex items-start justify-between gap-3">
                      <div class="min-w-0">
                    ...
                    <div class="mt-3 overflow-hidden rounded-lg border border-white/10 bg-black/30">
            ...
            <div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <label class="mt-4 block cursor-pointer rounded-2xl border border-dashed border-white/25 bg-black/20 p-6 hover:bg-black/30">
                ...
                <input type="file" multiple accept="image/*,application/pdf" class="hidden" @change="onPickFile(docType, $event)" />
                  ...
                  <div v-if="(advancedForm.files[docType] || []).length" class="mt-3 space-y-2">
                    <div v-for="(f, index) in advancedForm.files[docType]" :key="`${docType}-${index}-${f.name}`" class="rounded-xl border border-white/10 bg-black/25 p-3">
                      <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                          <div class="truncate text-sm text-white/90">{{ f.name }}</div>
                          <div class="mt-1 text-xs text-white/55">{{ formatBytes(f.size) }}</div>
                        </div>
                        <button type="button" class="text-xs text-white/60 hover:text-white" @click="clearFile(docType, index)">移除</button>
                      </div>
